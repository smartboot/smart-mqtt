...
public class BenchPlugin extends Plugin {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (SCENARIO_PUBLISH.equals(scenario)) {
                    ...
                    logger("[bench-plugin] 未知场景: " + scenario + ", 支持: publish, subscribe");
                }
            } catch (Throwable e) {
                throw new RuntimeException(e);
            } finally {
                logger("[bench-plugin] 压测结束");
                if (group != null) {
                    group.shutdown();
                }
            }
    ...
    private void runPublishBenchmark(PluginConfig config, AsynchronousChannelGroup group) throws InterruptedException {
        ...
        logger("[bench-plugin] 启动发布压测:");
        logger("  发布者数: " + connections);
        logger("  负载大小: " + payloadSize + " bytes");
        logger("  主题数: " + topicCount);
        logger("  每次发布数: " + publishCount);
        logger("  发布间隔: " + period + "ms");
        logger("  QoS: " + qos);
    ...
    private void runSubscribeBenchmark(BrokerContext brokerContext, PluginConfig config, AsynchronousChannelGroup group) throws Throwable {
        ...
        logger("[bench-plugin] 启动订阅压测:");
        logger("  订阅者数: " + connections);
        logger("  主题数: " + topicCount);
        logger("  QoS: " + qos);
        ...
        logger("[bench-plugin] 启动发布者数量: " + publisherCount);
