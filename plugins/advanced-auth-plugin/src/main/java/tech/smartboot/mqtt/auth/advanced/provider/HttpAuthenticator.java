...
public class HttpAuthenticator extends AbstractAuthenticator {
    ...
    protected void doInitialize(PluginConfig.AuthenticatorConfig config) {
        this.url = config.getStringOption("url", null);
        this.method = config.getStringOption("method", "POST");
        this.contentType = config.getStringOption("contentType", "application/json");
        this.timeout = config.getIntOption("timeout", 3000);
        this.usernameField = config.getStringOption("usernameField", "username");
        this.passwordField = config.getStringOption("passwordField", "password");
        this.tokenHeader = config.getStringOption("tokenHeader", null);
        this.successCode = config.getIntOption("successCode", 200);
        this.bodyTemplate = config.getStringOption("bodyTemplate", null);
        
        if (this.url == null || this.url.isEmpty()) {
            throw new IllegalArgumentException("HTTP authenticator URL is required");
        }
    }
