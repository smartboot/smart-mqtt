...
### Q: 压测时出现连接超时？
A: 检查系统文件描述符限制，确保足够大（建议 65535 以上）。

### Q: 压测结果不稳定？
A: 确保压测机和被测 Broker 在不同机器上，避免资源争抢。

### Q: 如何压测 WebSocket 连接？
A: 目前 smart-mqtt-bench 主要针对 TCP 连接，WebSocket 压测建议使用其他工具如 JMeter。

### Q: 压测时内存占用过高？
A: 可以启用低内存模式，或调整 JVM 堆内存大小。