...
# 发版记录
## v1.6.0（2026-06-06）
- smart-socket 升级至 2.0.0
- Feat 升级至 2.1.0
- 优化 RetainMessage 内存占用
- 更新内置试用版 License

> 本版本属于不兼容升级，建议升级前验证现有环境及自定义插件兼容性。

发版详情：《[smart-mqtt v1.6.0 ：AI 时代，一人产品的边界在哪里？](https://mp.weixin.qq.com/s/QFw6auvPrqTK3YRiwdtdCg)》

## v1.5.5（2026-04-11）

- **FlappingDetector 连接防抖检测机制**：新增滑动时间窗口统计与自动封禁策略，主动防御"抖动客户端"对 Broker 造成的连接风暴冲击
- **滑动时间窗口统计**：毫秒级精度的时间窗口统计连接事件，精准识别异常重连模式
- **高性能并发设计**：基于 ConcurrentHashMap + ConcurrentLinkedDeque 实现并发安全的统计机制
- **惰性过期清理**：封禁过期自动解除，后台任务定期清理冗余数据，内存开销可控
- **多场景防御**：覆盖网络不稳定边缘设备、客户端程序Bug、配置错误认证循环、DDoS攻击等典型异常场景

发版详情：《[smart-mqtt v1.5.5发布：主动防御连接风暴，让 Broker 更稳](https://mp.weixin.qq.com/s/5yGWQ0dZzSYZJi-1X1kr-g)》
